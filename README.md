...
Une banque fait des appels pour proposer un dépôt à terme. On veut prédire si un client va dire `yes` ou `no` — pour qu'on appelle en priorité ceux qui ont le plus de chances de souscrire.
...
Cible déséquilibrée : 88% `no` / 12% `yes` → l'accuracy seule ment, un modèle qui prédit toujours `no` fait déjà 88% sans rien apprendre. Métriques retenues : ROC-AUC + recall sur `yes`.
Split : 80/20 avec `stratify=y` et `random\\\_state=42`.
---
Preprocessing
...
Classes déséquilibrées : sous-échantillonner ou `class\\\_weight='balanced'` ?
